...
def combine_middlewares(
    ...
    """
    Returns a callable function which will call the provider_request_fn wrapped with all the middlewares.
    Middlewares are applied in reverse order, so the first middleware in the list will be the outermost.
    """
    return toolz.reduce(
